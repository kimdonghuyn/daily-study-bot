...
function buildPrompt(topic, type, diffGuide) {
  if (type === 'concept') {
    return `당신은 백엔드 시니어 개발자입니다. "${topic}"에 대한 깊이 있는 개념 질문을 만들어주세요.
${diffGuide}
규칙:
...
- 질문만 작성 (답변 포함 금지)`;
  ...
  if (type === 'problem') {
    return `당신은 백엔드 시니어 개발자입니다. "${topic}"과 관련된 실무 장애 시나리오를 만들어주세요.
${diffGuide}
...
- 질문만 작성 (답변 포함 금지)`;
  ...
  // interview
  return `당신은 백엔드 기술 면접관입니다. "${topic}"에 대한 기술 면접 질문을 만들어주세요.
${diffGuide}
규칙:
...
export async function generateDailyQuestion(topic, type, level) {
  const diffGuide = getDifficultyGuide(level);
  const question = await generate(buildPrompt(topic, type, diffGuide), 2000);
...
export { TOPIC_POOL, TYPE_LABELS, TYPE_SEQUENCE };
