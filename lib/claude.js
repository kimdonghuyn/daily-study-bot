...
export async function generateQuestion(topic, type) {
  const prompts = {
    concept: `당신은 백엔드 시니어 개발자입니다. "${topic}"에 대해 1시간 동안 깊이 생각하고 답변할 수 있는 개념 질문을 만들어주세요.
규칙:
- 단순 정의가 아닌 "왜", "어떻게", "어떤 상황에서" 를 묻는 질문
- 내부 동작 원리, 트레이드오프, 실무 적용까지 생각해야 완전한 답변이 나오는 깊이
- 질문만 작성 (답변 포함 금지)
- 3~4문장으로 맥락과 질문을 명확하게`,

    problem: `당신은 백엔드 시니어 개발자입니다. "${topic}"과 관련된 실무 문제 상황을 만들어주세요.
규칙:
- 실제로 일어날 법한 구체적인 장애/버그 시나리오
- 원인 분석 → 해결 방법 → 재발 방지까지 1시간 동안 고민해야 할 깊이
- 질문만 작성 (답변 포함 금지)
- 5~7문장으로 상황을 충분히 묘사한 뒤 질문`,

    interview: `당신은 네카라쿠배 수준 회사의 백엔드 면접관입니다. "${topic}"에 대한 심층 기술 면접 질문을 만들어주세요.
규칙:
- 표면적인 정의 말고 원리와 트레이드오프를 설명해야 하는 질문
- 꼬리 질문 2개를 함께 제시해 총 1시간 분량의 사고를 유도
- 질문만 작성 (답변 포함 금지)
- 메인 질문 1개 + 꼬리 질문 2개 형태로`,
  };

  const question = await generate(prompts[type], 500);
