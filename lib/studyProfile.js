...
import { TOPIC_POOL, TYPE_SEQUENCE, CYCLE_LENGTH, CURRICULUM } from './topics.js';
...
const TOPIC_SKILL = {
  // Java / OOP
  'JVM 메모리 구조 (Heap, Stack, Method Area)': 'java',
  'GC의 필요성과 동작 원리': 'java',
  'GC 알고리즘 종류 (Serial, G1, ZGC)': 'java',
  '캡슐화와 접근 제어자': 'java',
  '상속과 다형성': 'java',
  '추상화 — 인터페이스 vs 추상클래스': 'java',
  'SOLID — SRP와 OCP': 'java',
  'SOLID — LSP, ISP, DIP': 'java',
  '동시성 문제 — Race Condition과 Deadlock': 'java',
  // Spring
  'IoC와 DI — 왜 직접 객체를 만들지 않나': 'spring',
  'Spring Bean 생명주기와 스코프': 'spring',
  'Spring AOP — 어떤 문제를 해결하나': 'spring',
  '프록시 패턴과 AOP 내부 동작 원리': 'spring',
  '@Transactional 기본 동작': 'spring',
  '@Transactional 전파 수준과 롤백 규칙': 'spring',
  'Spring MVC — DispatcherServlet과 요청 흐름': 'spring',
  'Filter vs Interceptor vs AOP': 'spring',
  'JWT 구조와 인증 방식': 'spring',
  'OAuth2 인증 흐름 (Authorization Code)': 'spring',
  // DB
  '인덱스 개념 — 왜 빠른가': 'db',
  'B-Tree 인덱스 구조와 활용 전략': 'db',
  '트랜잭션과 ACID': 'db',
  '트랜잭션 격리 수준 4단계': 'db',
  'N+1 문제와 해결 방법': 'db',
  'Optimistic Lock vs Pessimistic Lock': 'db',
  'Redis란 무엇인가 — 언제 DB 대신 쓰나': 'db',
  'Redis 자료구조 5가지': 'db',
  '캐시 읽기 전략 (Look-aside, Read-through)': 'db',
  '캐시 쓰기 전략 (Write-through, Write-back)': 'db',
  // API
  'HTTP 기본 — 메서드, 상태코드, 헤더': 'msa',
  'REST API 설계 원칙과 멱등성': 'msa',
  // 메시징 / 아키텍처
  'Kafka — 왜 메시지 큐를 쓰나': 'kafka',
  'Kafka 구조 — Topic, Partition, Consumer Group': 'kafka',
  'MSA vs 모놀리식 트레이드오프': 'msa',
  'CAP 정리': 'msa',
  'Circuit Breaker 패턴': 'msa',
  // 인프라
  'Docker 개념 — VM과 차이, 왜 쓰나': 'aws',
  'Docker 이미지와 컨테이너 동작 방식': 'aws',
  'Kubernetes 기본 (Pod, Service, Deployment)': 'aws',
};
...
function renderCurriculumProgress(dayNumber) {
  const cycle = Math.floor(dayNumber / CYCLE_LENGTH);
  const cycleLabel = cycle === 0 ? '신입' : cycle === 1 ? '주니어' : '중급+';
  const dayInCycle = dayNumber % CYCLE_LENGTH;
  const bar = progressBar(dayInCycle, CYCLE_LENGTH);
  const pct = Math.round((dayInCycle / CYCLE_LENGTH) * 100);

  const currentTopicIdx = Math.floor(dayNumber / 3) % TOPIC_POOL.length;
  const currentTopic = TOPIC_POOL[currentTopicIdx];
  const TYPE_LABELS_KR = ['📖 개념', '🔧 문제', '🎤 면접'];
  const currentTypeLabel = TYPE_LABELS_KR[dayNumber % 3];

  // 카테고리별 요약 행
  const categoryRows = CURRICULUM.map(({ category, topics }) => {
    const startIdx = TOPIC_POOL.indexOf(topics[0]);
    const completedCount = topics.filter((_, ti) => (startIdx + ti) * 3 + 2 < dayNumber).length;
    const inProgress = topics.some((_, ti) => {
      const g = startIdx + ti;
      return g * 3 <= dayNumber && dayNumber < g * 3 + 3;
    });
    const icon = completedCount === topics.length ? '✅' : inProgress ? '🔄' : '⬜';
    const catBar = progressBar(completedCount, topics.length, topics.length);
    return `| ${icon} ${category} | ${completedCount} / ${topics.length} | \`${catBar}\` |`;
  }).join('\n');

  // 전체 토픽 상세 (접힘)
  const topicDetails = CURRICULUM.map(({ category, topics }) => {
    const startIdx = TOPIC_POOL.indexOf(topics[0]);
    const rows = topics.map((topic, ti) => {
      const g = startIdx + ti;
      const marks = [0, 1, 2].map(j => {
        const pos = g * 3 + j;
        if (pos < dayNumber) return '✅';
        if (pos === dayNumber) return '🔄';
        return '⬜';
      });
      return `| ${topic} | ${marks[0]} | ${marks[1]} | ${marks[2]} |`;
    }).join('\n');
    return `### ${category}\n| 토픽 | 📖 | 🔧 | 🎤 |\n|------|:--:|:--:|:--:|\n${rows}`;
  }).join('\n\n');
...
**Cycle ${cycle} · ${cycleLabel} 기준** | Day ${dayInCycle} / ${CYCLE_LENGTH} | \`${bar}\` ${pct}%

> 🔄 현재: **${currentTopic}** — ${currentTypeLabel}
...
${renderCurriculumProgress(profile.dayNumber || 0)}
...
export async function regenerateFilesFromProfile() {
  ...
  await putFile('profile.md', renderProfileMd(profile), 'reset: 프로필 재생성', profileMd?.sha);

  const readmeFile = await getFile('README.md');
  await putFile('README.md', renderReadme(profile, profile.recentLogs || []), 'reset: README 재생성', readmeFile?.sha);

  await updateGitHubProfile(profile);

  console.log('✅ README + profile.md + GitHub 프로필 재생성 완료');
}
...
export async function updateStudyProfile({ date, topic, questions, userAnswer, feedback, modelAnswers, answerScore }) {
  ...
  // dayNumber는 /study-done 실행 시에만 증가 (updateStudyProfileFromSession)
  if (userAnswer) profile.questsCompleted += questions.length;
